...
//    $order_total_after_tax = 0;
          $statement = $connect->query("SELECT (max(idordencompra)+1)idordencompra from ordencompracabecera");
           $idordencompra = $statement->fetchColumn();
         
    $statement = $connect->prepare("
      
      INSERT INTO ordencompracabecera 
        (idordencompra,fecha,vencimiento,notas,RucProveedor,idCentroDeCosto,TerminosDePago,idPresupuesto,SitioEntrega,FechaEntrega )
        VALUES (:idordencompra,:from, :to, :guardar,:prov,:combocentrodecostosorden,:comboterminosdepagoorden,:pres,:txtssitios,:fecha)
    ");
    ...
    $statement->execute(
      array(
          ...
          ':from'               =>  trim($_POST["from"]),
           ...
           ':combocentrodecostosorden'               =>  trim($_POST["combocentrodecostosorden"]),
           ':comboterminosdepagoorden'               =>  trim($_POST["comboterminosdepagoorden"]),
           ':pres'               =>  trim($_POST["pres"]),
           ':txtssitios'               =>  trim($_POST["txtssitios"]),
...
$id = uniqid();
        $statement = $connect->prepare("
          ...
          (idordencompra, codigoproducto, cantidad,precio,id)
          VALUES (:idordencompra, :item_cod, :order_item_price, :order_item_quantity,:id)
        ");
...
//      );
//      
      header("location:vistaorden.php?orden=$idordencompra");
