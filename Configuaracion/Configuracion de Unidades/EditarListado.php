...
<html>
       <head>
        ...
        <a href=></a>
     <title>Editar Listado de Precio</title>
    <meta charset="utf-8">
...
<!--    <div class="container-fluid">-->
      ...
      elseif(isset($_GET["updatep"]) && isset($_GET["idListado"]))
      {
        $statement = $connect->prepare("
          SELECT * FROM listados
            WHERE idListado = :idListado group by idListado
           
        ");
        $statement->execute(
          array(
            ':idListado'       =>  $_GET["idListado"]
            )
          );
        $result = $statement->fetchAll();
        foreach($result as $row)
...
/*                                font-weight:  bold;*/
     ...
     <body id="hhmenu">
      ...
      <div id="header">
       ...
       <form id="getactionorden" name="getactionorden" method="post"action="actionlistado.php">
            <input type="hidden" name="accion" value="registrarproduccion" />
            <h4><a style="color: black" href="Reportes de Compras.jsp">Crear Listado de Precio</a></h4>
...
<div class="form-group" style="margin-left:0px;margin-top:50px;">  
          <span class="help-block" id="error"></span>    
         ...
         <table style="margin-left:0px;" id="invoice-item-table" class="table table-bordered">
                     <tr>
                      ...
                      <th width="20%">Producto.</th>
                       <th width="20%">Precio Por Envase.</th>
                       <th width="20%">Cantidad Por Envase.</th>
                    ...
                    SELECT * FROM listados
                      ...
                      WHERE idlistado = :idListado order by idListado
                    ");
                    $statement->execute(
                      array(
                        ':idListado'       =>  $_GET["idListado"]
                   ...
                   <tr>
                    ...
                    <td><input type="text" name="order_item_quantity[]" id="order_item_quantity<?php echo $m; ?>" data-srno="<?php echo $m; ?>" class="form-control input-sm order_item_quantity" value = "<?php echo $sub_row["precioporenvase"]; ?>"/></td>

                      <td><input type="text" name="order_item_price[]" id="order_item_price<?php echo $m; ?>" data-srno="<?php echo $m; ?>" class="form-control input-sm number_only order_item_price" value="<?php echo $sub_row["cantidadporenvase"]; ?>" /></td>
           

                      <td><input type="text" name="order_item_um[]" id="order_item_um<?php echo $m; ?>" data-srno="<?php echo $m; ?>" class="form-control input-sm order_item_um" value = "<?php echo $sub_row["unidaddemedida"]; ?>"/></td>
     ...
     <?php echo $producto; ?>
              ...
              <tr>
                <td colspan="2" align="center">
                  ...
                  <input type="hidden" name="idListado" id="idListado <?php echo $m; ?>" value="<?php echo $row["idListado"]; ?>" />
