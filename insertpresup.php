...
//    $order_total_after_tax = 0;
          $statement = $connect->query("SELECT (max(idPresupuesto)+1)idPresupuesto from presupuestocabecera");
           $idPresupuesto = $statement->fetchColumn();
                  
    $statement = $connect->prepare("
      INSERT INTO presupuestocabecera
        (idPresupuesto,RucProveedor,Fecha,Vencimiento,idCentroDeCosto,Notas,idPedidoCompra)
        VALUES (:idPresupuesto,:RucProveedor,:fecha, :vencimient,:combcentropresup, :guardar,:cbopedido)
    ");
    ...
    $statement->execute(
      array(
           ':idPresupuesto'               =>  $idPresupuesto,
            ':RucProveedor'               =>  trim($_POST["comboproveedorp"]),
          ':fecha'               =>  trim($_POST["fecha"]),
           ':vencimient'               =>  trim($_POST["vencimient"]),
           ':guardar'               =>  trim($_POST["guardar"]),
            ':combcentropresup'               =>  trim($_POST["combcentropresup"]),
             ':cbopedido'               =>  trim($_POST["cbopedido"])
...
$id = uniqid();
        $statement = $connect->prepare("
          ...
          (idPresupuesto, codigoproducto, cantidad,precio,id)
          VALUES (:idPresupuesto, :item_cod, :order_item_price, :order_item_quantity,:id)
        ");
...
//      );
//      
      header("location:VistaPresupuesto.php?presupuesto=$idPresupuesto");
